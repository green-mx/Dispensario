<template>
  <v-container fluid>
    <!-- Header imagen: mismo patrón que OtrosView (con fallback si la
         imagen del banner todavía no existe en /public/assets) -->
    <v-row class="mb-4">
      <v-col cols="12">
        <v-card>
          <v-img src="/assets/frascos-banner.jpg" height="220px" contain class="header-img">
            <template #error>
              <div class="banner-img-fallback"></div>
            </template>
            <div class="header-overlay d-flex align-center justify-center">
              <h1 class="header-title">FRASCOS</h1>
            </div>
          </v-img>
        </v-card>
      </v-col>
    ...
    <!-- Explorador: filtros + carrusel + detalle, mismo patrón que
         WeedView/OtrosView. Los filtros salen solos de las subcategorías
         que traigan los productos (si solo hay una, queda nomás "Todos"). -->
...
onMounted(async () => {
  try {
    products.value = await fetchProductosPorCategoria('frascos');
  } catch (e) {
