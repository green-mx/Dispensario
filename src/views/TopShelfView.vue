<template>
  <v-container fluid>
    <!-- Header imagen: mismo patrón que WeedView (con fallback si la
         imagen del banner todavía no existe en /public/assets) -->
    <v-row class="mb-4">
      <v-col cols="12">
        <v-card>
          <v-img src="/assets/top-shelf-banner.jpg" height="220px" contain class="header-img">
            <template #error>
              <div class="banner-img-fallback"></div>
            </template>
            <div class="header-overlay d-flex align-center justify-center">
              <h1 class="header-title">TOP SHELF</h1>
            </div>
          </v-img>
        </v-card>
      </v-col>
    ...
    <!-- Explorador: filtros + carrusel + detalle, mismo patrón que las
         demás vistas de catálogo. Los filtros salen solos de las
         subcategorías que traigan los productos. -->
...
onMounted(async () => {
  try {
    products.value = await fetchProductosPorCategoria('top-shelf');
  } catch (e) {
